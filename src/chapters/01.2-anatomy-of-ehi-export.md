...
Every table in your export has built-in documentation. Epic embeds table and column descriptions directly in the schema:

<example-query description="View documentation for a reference table">
-- SQLite preserves Epic's documentation as comments
-- Let's look at the department table
SELECT sql 
FROM sqlite_master 
WHERE type='table' 
  AND name='CLARITY_DEP';
</example-query>

When working with the raw TSV files, you'll find this same documentation in the accompanying PDF data dictionary. The documentation typically includes:
- Table purpose and contents
- Column definitions
