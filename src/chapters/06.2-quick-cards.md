...
SELECT 
  ...
  ha.ACCT_BILLSTS_HA as billing_status,
  ha.TOT_CHGS as total_charges,
  ha.TOT_PMTS as total_payments,
  ha.TOT_ADJS as total_adjustments,
  ha.ACCT_BALANCE as current_balance
FROM HSP_ACCOUNT ha
...
WHERE op.PAT_ID = 'Z7004242'
  AND op.ORDER_TYPE = 3  -- Lab orders
  AND res.ORD_NUM_VALUE IS NOT NULL
ORDER BY res.RESULT_TIME DESC
LIMIT 20;
</example-query>
...
SELECT 
  ...
  pm.PAYOR_NAME,
  pm.PLAN_NAME,
  c.SUBSCR_MEMBER_ID as member_id,
  c.EFF_DATE,
  c.TERM_DATE
FROM COVERAGE c
JOIN PAYOR_PLAN_MAP pm ON c.PAYOR_ID = pm.PAYOR_ID
WHERE c.PAT_ID = 'Z7004242'
  AND (c.TERM_DATE IS NULL OR c.TERM_DATE > date('now'))
ORDER BY c.FILING_ORDER;
</example-query>
...
SELECT 
  adt.EFFECTIVE_TIME,
  adt.EVENT_TYPE,
  from_dep.DEPARTMENT_NAME as from_location,
  to_dep.DEPARTMENT_NAME as to_location,
  adt.ROOM_ID,
  adt.BED_ID
FROM CLARITY_ADT adt
LEFT JOIN CLARITY_DEP from_dep ON adt.FROM_DEP_ID = from_dep.DEPARTMENT_ID
LEFT JOIN CLARITY_DEP to_dep ON adt.TO_DEP_ID = to_dep.DEPARTMENT_ID
