...
Every day, healthcare systems process thousands of laboratory tests - from simple glucose checks to complex genetic panels. Each test generates multiple data points: the order, the results, reference ranges, abnormal flags, and clinical interpretations. Epic's laboratory domain elegantly manages this complexity through interconnected tables that maintain both clinical context and international standards.

Let's explore this system by following a patient's actual laboratory journey, discovering how Epic transforms a provider's order into actionable clinical data.
...
<example-query description="Identify any critical lab values">
SELECT 
    op.ORDERING_DATE,
    op.DESCRIPTION as TEST,
    cc.NAME as COMPONENT,
    ore.ORD_VALUE || ' ' || ore.REFERENCE_UNIT as RESULT,
    ore.RESULT_FLAG_C_NAME,
    'REQUIRES IMMEDIATE CLINICAL REVIEW' as ACTION
FROM ORDER_PROC op
JOIN ORDER_RESULTS ore ON op.ORDER_PROC_ID = ore.ORDER_PROC_ID
JOIN CLARITY_COMPONENT cc ON ore.COMPONENT_ID = cc.COMPONENT_ID
WHERE op.PAT_ID = 'Z7004242'
  AND ore.RESULT_FLAG_C_NAME IN ('Critical High', 'Critical Low', 'Panic')
ORDER BY op.ORDERING_DATE DESC;
</example-query>

While our sample patient has no critical values, this query pattern is essential for clinical surveillance systems.
...
SELECT 
    op.ORDERING_DATE,
    pe.ENC_TYPE_C_NAME as ENCOUNTER_TYPE,
    op.DESCRIPTION,
...
WHERE op.PAT_ID = 'Z7004242'
  AND op.ORDER_TYPE_C_NAME = 'Lab'
GROUP BY op.ORDER_PROC_ID, op.ORDERING_DATE, pe.ENC_TYPE_C_NAME, op.DESCRIPTION
ORDER BY op.ORDERING_DATE DESC
LIMIT 10;
</example-query>
