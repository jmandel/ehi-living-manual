...
SELECT 
  ...
  pl.PROBLEM_NAME,
  pl.ICD10_CODE,
  DATE(pl.ONSET_DATE) as onset_date,
  pl.PROBLEM_STATUS_C_NAME as status,
  pl.PRIORITY_C_NAME as priority,
  pl.PROBLEM_CLASS_C_NAME as class,
  DATE(pl.RESOLVE_DATE) as resolved_date
FROM PROBLEM_LIST pl
WHERE pl.PAT_ID = 'Z7004242'
  AND pl.PROBLEM_STATUS_C_NAME = 'Active'
ORDER BY pl.ONSET_DATE DESC;
</example-query>
...
SELECT 
  imm.IMMUNE_ID,
  ci.GENERIC_NAME_ as vaccine_name,
  DATE(imm.IMMUNE_DATE) as given_date,
  imm.DOSE,
  imm.DOSE_UNIT_C_NAME,
  imm.ROUTE_C_NAME,
  imm.SITE_C_NAME,
  imm.LOT,
  imm.MANUFACTURER_C_NAME
FROM IMMUNE imm
JOIN CLARITY_IMMUNZATN ci ON imm.IMMUNZATN_ID = ci.IMMUNZATN_ID
WHERE imm.PAT_ID = 'Z7004242'
...
SELECT 
  ...
  DATE(ht.POST_DATE) as post_date,
  ht.TX_TYPE_C_NAME as transaction_type,
  ht.AMOUNT,
  ht.BUCKET_ID,
  ht.FIN_CLASS_C_NAME
FROM HSP_TRANSACTIONS ht
JOIN HSP_ACCOUNT ha ON ht.HSP_ACCOUNT_ID = ha.HSP_ACCOUNT_ID
WHERE ha.PAT_ID = 'Z7004242'
ORDER BY ht.POST_DATE DESC, ht.TX_ID;
</example-query>
...
WITH date_checks AS (
  ...
  WHERE res.RESULT_DATE < op.ORDER_DATE
...
WITH utilization_summary AS (
  SELECT 
    ...
    dep.SPECIALTY_C_NAME,
    COUNT(*) as visit_count,
    COUNT(DISTINCT DATE(pe.CONTACT_DATE)) as unique_days
  FROM PAT_ENC pe
  LEFT JOIN CLARITY_DEP dep ON pe.DEPARTMENT_ID = dep.DEPARTMENT_ID
  WHERE pe.CONTACT_DATE >= date('now', '-2 years')
  GROUP BY pe.PAT_ID, year, dep.SPECIALTY_C_NAME
)
SELECT 
  PAT_ID,
  year,
  SUM(visit_count) as total_visits,
  COUNT(DISTINCT SPECIALTY_C_NAME) as specialties_seen,
  SUM(unique_days) as total_visit_days,
  GROUP_CONCAT(SPECIALTY_C_NAME || ' (' || visit_count || ')', ', ') as specialty_breakdown
