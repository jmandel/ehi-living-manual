...
SELECT 
  ...
  SERVICE_DATE,
  TX_POST_DATE,
  DATEDIFF('day', SERVICE_DATE, TX_POST_DATE) as days_to_post,
  -- Where (bucket)
  BUCKET_ID,
  -- Clinical context
...
SELECT 
  *,
  ...
  DATEDIFF('day', first_service, last_service) as service_span_days
...
SELECT 
  ...
  SERVICE_DATE,
  TX_POST_DATE,
  DATEDIFF('day', SERVICE_DATE, TX_POST_DATE) as days_to_payment,
  BUCKET_ID,
  -- Identify payment source by amount and timing
  CASE 
    WHEN DATEDIFF('day', SERVICE_DATE, TX_POST_DATE) < 30 THEN 'Quick pay'
    WHEN DATEDIFF('day', SERVICE_DATE, TX_POST_DATE) < 60 THEN 'Standard pay'
    WHEN DATEDIFF('day', SERVICE_DATE, TX_POST_DATE) < 120 THEN 'Slow pay'
    ELSE 'Very slow pay'
