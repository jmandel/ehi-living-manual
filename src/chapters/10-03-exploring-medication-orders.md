...
SELECT 
    ...
    mci.COVERAGE_ID
FROM ORDER_MED om
JOIN MED_CVG_INFO mci ON om.ORDER_MED_ID = mci.ORDER_MED_ID
...
SELECT 
    ...
    CASE 
        WHEN mci.ORDER_MED_ID IS NOT NULL THEN 'Coverage Checked'
        ELSE 'No Coverage Info'
    END as COVERAGE_STATUS
FROM ORDER_MED om
LEFT JOIN CLARITY_MEDICATION cm ON om.MEDICATION_ID = cm.MEDICATION_ID
LEFT JOIN ORDER_MED_SIG oms ON om.ORDER_MED_ID = oms.ORDER_ID
LEFT JOIN MED_CVG_INFO mci ON om.ORDER_MED_ID = mci.ORDER_MED_ID
WHERE om.PAT_ID = 'Z7004242'
ORDER BY om.ORDERING_DATE DESC;
</example-query>
