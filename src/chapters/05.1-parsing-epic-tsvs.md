...
SELECT 
  ...
  SUM(CASE WHEN LENGTH(PLAIN_TEXT) > 100 THEN 1 ELSE 0 END) as long_notes
FROM HNO_PLAIN_TEXT
...
SELECT 
  ...
  COUNT(CASE WHEN ENC_TYPE_C IS NULL THEN 1 END) as missing_type
FROM PAT_ENC;
</example-query>
...
SELECT 
  ...
  COUNT(*) as total_rows,
  COUNT(DISTINCT PAT_ID) as unique_patients,
  COUNT(CASE WHEN ENC_TYPE_C_NAME LIKE '%,%' THEN 1 END) as fields_with_commas,
