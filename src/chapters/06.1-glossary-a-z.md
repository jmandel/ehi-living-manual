...
-- See how accounts link to guarantors and patients
SELECT 
  a.ACCOUNT_ID,
  a.ACCOUNT_NAME,
  a.GUARANTOR_ID,
...
FROM ACCOUNT a
LEFT JOIN ACCT_GUAR_PAT_INFO ap ON a.GUARANTOR_ID = ap.GUARANTOR_ID
GROUP BY a.ACCOUNT_ID, a.ACCOUNT_NAME, a.GUARANTOR_ID
LIMIT 5;
</example-query>
...
SELECT 
  ...
  MEMBER_ID
FROM COVERAGE
WHERE FILING_ORDER_C_NAME IN ('Primary', 'Secondary')
ORDER BY PAT_ID, FILING_ORDER_C_NAME;
</example-query>
...
SELECT 
  g.GUARANTOR_ID,
  COUNT(DISTINCT p.PAT_ID) as patients_covered,
  g.ACCOUNT_NAME
FROM ACCOUNT g
JOIN ACCT_GUAR_PAT_INFO p ON g.GUARANTOR_ID = p.GUARANTOR_ID
GROUP BY g.GUARANTOR_ID, g.ACCOUNT_NAME
HAVING COUNT(DISTINCT p.PAT_ID) > 1;
</example-query>
...
SELECT 
  ...
  ACCT_BASECLS_HA_C_NAME as patient_class
FROM HSP_ACCOUNT
LIMIT 5;
</example-query>
...
SELECT 
  ...
  DX_ID
FROM PAT_ENC_DX
WHERE PAT_ENC_CSN_ID = 207980866
ORDER BY LINE;
</example-query>
...
SELECT 
  ...
  IDENTITY_TYPE_ID
FROM IDENTITY_ID
WHERE IDENTITY_TYPE_ID = 14  -- Type 14 is often MRN
LIMIT 5;
</example-query>
